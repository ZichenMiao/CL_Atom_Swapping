...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, num_bases=12, num_member=1):
        super(ResNet, self).__init__()
        self.in_planes = 16
        self.block = block
        self.layers = num_blocks
        self.stride_list = [1, 2, 2]
        self.planes_list = [16, 32, 64]
        ...
        self.out_size = self.planes_list[-1] * block.expansion
        ...
        for l, planes in enumerate(self.planes_list):
            for b in range(self.layers[l]):
                ## conv1 coeff
                self.coeff_list.append(Parameter(torch.Tensor(planes, inplanes*num_bases, 1, 1)))
                ## update inplanes
                if b == 0:
                    inplanes = planes * self.block.expansion
                
                ## conv2 coeff
                self.coeff_list.append(Parameter(torch.Tensor(planes, planes*num_bases, 1, 1)))
